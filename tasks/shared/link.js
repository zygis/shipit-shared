...
module.exports = function (gruntOrShipit) {
  ...
  function link(filePath) {
    ...
    return shipit.remote(
      sprintf('if ( ! [ -h "%(target)s" ] ) || ( [ -h "%(target)s" ] && [ $(readlink -n "%(target)s" ) != "%(source)s" ] ); then mv %(target)s/* %(source)s/; mv %(target)s/.* %(source)s/; rm -r "%(target)s" 2> /dev/null; ln -s "%(source)s" "%(target)s"; fi', {
